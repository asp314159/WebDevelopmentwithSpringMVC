...
import aka.springframework.webdevelopmentwithspringmvc.domain.*;
...
@Slf4j
@Component
public class InsertingRecipes implements ApplicationListener<ContextRefreshedEvent> {
    ...
    //To Convert Optional<T> to List or Set -- Example can be found in Index Controller
    ...
    public List<Recipe> getRecipe() {
        log.debug("Creating Recipes object");
        List<Recipe> recipes = new ArrayList<>(2);

        //get UOMs
        UnitOfMeasure eachUom = unitOfMeasureRepository.findByDescription("Each").orElseGet(() -> {throw new RuntimeException("Expected UOM Not Found");});
        UnitOfMeasure tableSpoonUom = unitOfMeasureRepository.findByDescription("Tablespoon").orElseGet(() -> {throw new RuntimeException("Expected UOM Not Found");});
        UnitOfMeasure teaSpoonUom = unitOfMeasureRepository.findByDescription("Teaspoon").orElseGet(() -> {throw new RuntimeException("Expected UOM Not Found");});
        UnitOfMeasure dashUom = unitOfMeasureRepository.findByDescription("Dash").orElseGet(() -> {throw new RuntimeException("Expected UOM Not Found");});
        UnitOfMeasure pintUom = unitOfMeasureRepository.findByDescription("Pint").orElseGet(() -> {throw new RuntimeException("Expected UOM Not Found");});
        UnitOfMeasure cupsUom = unitOfMeasureRepository.findByDescription("Cup").orElseGet(() -> {throw new RuntimeException("Expected UOM Not Found");});

        //get Categories
        Category americanCategory = categoryRepository.findByDescription("American").orElseGet(() -> {throw new RuntimeException("Expected UOM Not Found");});
        Category mexicanCategory = categoryRepository.findByDescription("Mexican").orElseGet(() -> {throw new RuntimeException("Expected UOM Not Found");});
